    ...
    switch ($action) {
        case 'deliver':
            if (is_post()) {
                ...
                $verification = $_user->createVerification(\Models\Verification::TYPE_EMAIL, $email, \Models\Verification::METHOD_EMAIL);

                try {
                    $delivered = $verification->deliver();
                    if ($delivered) {
                        // redirect i.e. refresh the page
                        redirect(APP_URL.'/php_security_email_verify.php');
                    } else {
                        $verification = null;
                        throw new Exception('Failed to deliver the verification email!');
                    }
                } catch (Exception $ex) {
                    $verification = null;
                    throw $ex;
        ...
        case 'validate':
            ...
            // $_user->verifyEmail($key);

            if ($key && $verification && $verification->validate($_user)) {
                $verified = $verification->verify($key);
                if ($verified) {
                    // redirect to somewhere for a successful verification
                    redirect(APP_URL.'/php_security_email_verify.php?verified=1');
                } else {
                            ...
                            <div class="row">
                                <div class="col-xl-6 ml-auto mr-auto">
                                    ...
                                    <div class="card p-4 rounded-plus bg-faded">
                                        <?php if ($verification) : ?>
                                        <h4 class="mb-3">Thank you! Please check your email.</h4>
                                        <div class="alert alert-primary text-dark" role="alert">
                                            We’ve sent a message to <strong><?= $verification->to ?></strong> with a link to activate your account.
                                        </div>
                                        <div>
                                            <a href="<?= APP_URL ?>/php_security_email_verify.php?action=reset&email=<?= urlencode($verification->to) ?>">
                                                <i class="fal fa-chevron-right mr-2"></i> Didn’t get a code?
                                            </a>
                                        </div>
                                        <?php else : ?>
                                        <form method="POST" action="<?= APP_URL.'/php_security_email_verify.php?action=deliver' ?>">
                                            <?= \App\Content::printPostToken(APP_URL.'/php_security_email_verify.php?action=deliver') ?>
                                            <h4 class="mb-3">For demo purposes, enter the email you want to receive the confirmation.</h4>
                                            <div class="alert alert-warning text-dark" role="alert">
                                                Normally, you would immediately deliver the account verification email right after registration.
                                            </div>
                                            <?php
