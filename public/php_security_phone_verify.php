...
if ($authenticated) {
    $verification = $_user->getVerification(\Models\Verification::TYPE_PHONE);
}
    ...
    switch ($action) {
        case 'deliver':
            if (is_post()) {
                ...
                // $phone = $_user->phone;
                $phone = phone(get('phone', $_POST)); // formatted

                $verification = $_user->createVerification(\Models\Verification::TYPE_PHONE, $phone, \Models\Verification::METHOD_SMS);

                try {
                    $delivered = $verification->deliver();
                    if ($delivered) {
                        // redirect i.e. refresh the page
                        redirect(APP_URL.'/php_security_phone_verify.php');
                    } else {
            ...
            if (is_post()) {
                ...
                if ($code && $verification && $verification->validate($_user)) {
                    ...
                    if ($verified) {
                        ...
                        redirect(APP_URL.'/php_security_phone_verify.php?verified=1');
                    } else {
                            ...
                            <div class="row">
                                <div class="col-xl-6 ml-auto mr-auto">
                                    ...
                                    <?php if ($authenticated) : ?>
                                        <?= get('verified') ? \App\Content::printAlert('<strong>Verification Success!</strong><br>Your verificaton link is valid. Good job!', 'success') : null ?>
                                    <div class="card p-4 rounded-plus bg-faded">
                                        <?php if ($verification) : ?>
                                        <form method="POST" action="<?= APP_URL.'/php_security_phone_verify.php?action=validate' ?>">
                                            <?= \App\Content::printPostToken(APP_URL.'/php_security_phone_verify.php?action=validate') ?>
                                            ...
                                            <div class="mt-3">
                                                <a href="<?= APP_URL ?>/php_security_phone_verify.php?action=reset&phone=<?= urlencode($verification->to) ?>">
                                                    <i class="fal fa-chevron-right mr-2"></i> Didn’t get a code?
                                                </a>
                                            </div>
                                        </form>
                                        <?php else : ?>
                                        <form method="POST" action="<?= APP_URL.'/php_security_phone_verify.php?action=deliver' ?>">
                                            <?= \App\Content::printPostToken(APP_URL.'/php_security_phone_verify.php?action=deliver') ?>
                                            <h4 class="mb-3">For demo purposes, enter the phone you want to receive the confirmation.</h4>
                                            <div class="alert alert-warning text-dark" role="alert">
                                                Normally, you would immediately deliver the verification code right after the phone is entered by the user during registration or something.
                                            </div>
                                            <?php

                                            $input = new \Bootstrap\Components\Input('phone', ['required' => true]);
                                            $input->label('Enter Phone Number')
                                                ->value(get('phone', $defaults))
                                                ->attr(['autofocus'])
                                                ->printHtml();
